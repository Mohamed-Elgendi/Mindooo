...
// THE CONTEXT ENGINE — loads all personal data before every AI call
...
async function loadStats(userId) {
  ...
  const [chroniclesRes, focusRes, allRes] = await Promise.all([
  ...
  const avgChaos   = chronicles.length ? Math.round(chronicles.reduce((s,c) => s+(c.chaos_score||0),0)/chronicles.length) : 0;
  const stats = {
    dumpsThisWeek:     chronicles.length,
    focusMinsThisWeek: Math.round(focus.reduce((s,f) => s+(f.actual_secs||0),0)/60),
    totalChronicles:   allRes.count || 0,
    clarityScore:      Math.max(0, 100 - avgChaos),
    avgChaosScore:     avgChaos,
  };
  cacheSet(`stats_${userId}`, stats);
  return stats;
}

async function loadRecentChronicles(userId, limit = 5) {
  const cached = cacheGet(`recent_${userId}`);
  if (cached !== null) return cached;
  const { data } = await supabase
    .from("chronicles")
    .select("title,text,ai_summary,chaos_score,emotional_tone,themes,created_at,origin")
    .eq("user_id", userId)
    .order("created_at", { ascending: false })
    .limit(limit);
  const result = data || [];
  cacheSet(`recent_${userId}`, result);
  return result;
}

function formatAboutMe(about) {
  if (!about) return "About Me profile: not yet completed.";
  const s = [];
  if (about.employment_status || about.location) s.push(`SITUATION: ${[about.employment_status, about.location].filter(Boolean).join(" · ")}`);
  if (about.main_constraints?.length) s.push(`CONSTRAINTS: ${about.main_constraints.join(", ")}`);
  if (about.work_preference || about.learning_style) s.push(`PERSONALITY: Works ${about.work_preference||"?"} · Learns by ${about.learning_style||"?"} · Decides via ${about.decision_style||"?"}`);
  if (about.peak_hours) s.push(`ENERGY PEAKS: ${about.peak_hours}`);
  ...
  if (about.ikigai_statement) s.push(`IKIGAI STATEMENT: ${about.ikigai_statement}`);
  if (about.top_values?.length) s.push(`VALUES: ${about.top_values.slice(0,5).join(", ")}`);
  if (about.freedom_definition) s.push(`FREEDOM MEANS: ${about.freedom_definition}`);
  const blockers = [];
...
function formatRecentChronicles(chronicles) {
  if (!chronicles.length) return "No recent brain dumps this week.";
  return chronicles.map((c, i) => {
    ...
    const preview = c.text?.slice(0, 200) || "(voice note or empty)";
...
export async function buildContext(userId) {
  if (!userId) return { systemPromptAddition:"", stats:null };
  const [aboutMe, cognitive, stats, recentChronicles] = await Promise.all([
    loadAboutMe(userId),
...
export function buildFullSystemPrompt(context, engineId, firstName, engineMap) {
  ...
  const { aboutMeText="", cognitiveText="", recentChroniclesText="", stats=null } = context || {};

  const statsText = stats ? `
PERFORMANCE THIS WEEK:
  Focus: ${((stats.focusMinsThisWeek||0)/60).toFixed(1)}h · Brain Dumps: ${stats.dumpsThisWeek||0} · Total Chronicles: ${stats.totalChronicles||0} · Clarity Score: ${stats.clarityScore||0}%` : "";
...
═══════════════════════════════════════════
${name.toUpperCase()}'S RECENT BRAIN DUMPS
═══════════════════════════════════════════
${recentChroniclesText}

When relevant, reference these chronicles specifically.
Say "3 days ago you wrote about..." or "your dump on Monday showed..."
Never invent what ${name} wrote — only reference what is shown above.
${engineSection}
...
3. Challenge ${name} when he needs it — with evidence, not flattery.
4. If lacking data, ask ONE specific question to get it.
5. Never invent patterns — only reference what is real.
6. End every response with ONE clear, specific, achievable next action.
7. You are the most intelligent, most personalised advisor ${name} has ever had. Respond accordingly.`;
}

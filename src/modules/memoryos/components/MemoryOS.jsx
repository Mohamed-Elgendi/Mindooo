...
// Root component — mount this in Dashboard.jsx
...
export function MemoryOS({ user, supabase, onNavigate }) {
  const m = useMemoryOS({
    ...
    initialScreen:  'home',
    onEvent: (event) => {
      // Forward events to parent platform if needed
      // console.log('[MemoryOS Event]', event.type, event.payload);
    },
  });
  ...
  switch (m.screen) {
    ...
    case 'guru-center':
      return <GuruCenterScreen {...screenProps} />;

    case 'guru-detail':
      return <GuruDetailScreen {...screenProps} />;

    case 'guru-step':
      return <GuruStepScreen {...screenProps} />;

    case 'knowledge-archive':
      return <ArchiveScreen {...screenProps} />;

    case 'curriculum-detail':
      return <CurriculumScreen {...screenProps} />;

    case 'session':
      return m.session.state.sessionDone
        ? <SessionDoneScreen {...screenProps} />
        : <SessionScreen {...screenProps} />;

    case 'session-done':
      return <SessionDoneScreen {...screenProps} />;

    case 'nobel-mind':
      return <NobelMindScreen {...screenProps} />;

    case 'nobel-domain':
      return <NobelDomainScreen {...screenProps} />;

    case 'ai-mind-lab':
      return <AIMindLabScreen {...screenProps} />;

    case 'ai-coach':
      return <AiCoachScreen {...screenProps} />;

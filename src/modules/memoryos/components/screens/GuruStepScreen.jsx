...
import { TopBar, BackButton, COLORS, PrimaryBtn, GhostBtn } from '../shared/UI.jsx';
...
export function GuruStepScreen({ m }) {
  ...
  async function handleAskGuru() {
    ...
    setAiLoading(true);
    try {
      const response = await getGuruStepGuidance(guru.name, step.title, step.lesson, aiInput, guru.aiSystemPrompt, m.activeCurriculumId || 'user');
      setAiGuide(response);
    } catch { setAiGuide('Unable to connect to AI right now. Use the pre-written guidance above.'); }
    finally { setAiLoading(false); }
  }

  async function markComplete() { setCompleted(true); await m.completeGuruStep(guru.id, step.id); }

  function goNext() {
    if (nextStep) {
      m.setActiveGuruStep(nextStep.id);
      m.navigate('guru-step', { guruId: guru.id, stepId: nextStep.id });
      setPhase('lesson'); setAiGuide(''); setAiInput(''); setCompleted(false);
    } else m.navigate('guru-detail', { guruId: guru.id });
  }

  return (
    <div className="section-scroll">
      <div className="section-content">
        <TopBar left={<BackButton onClick={() => m.navigate('guru-detail', { guruId })} />} center={
        ...
        <div style={{ background:color+'10', border:`1px solid ${color}33`, borderRadius:16, padding:'18px 18px', marginBottom:18 }}>
          <div style={{ display:'flex', gap:10, alignItems:'center', marginBottom:10 }}>
            <div style={{ width:36, height:36, borderRadius:'50%', background:color+'22', border:`1.5px solid ${color}`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:13, fontWeight:800, color, flexShrink:0 }}>{step.stepNumber}</div>
            <div>
          ...
          <div style={{ display:'flex', gap:6 }}>
            {['lesson','action'].map(p => (
              <button key={p} onClick={() => setPhase(p)} style={{ flex:1, padding:'8px', borderRadius:8, border:`1px solid ${phase===p?color:COLORS.border2}`, background:phase===p?color+'22':COLORS.surface2, color:phase===p?color:'#555', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>
        ...
        {phase === 'lesson' && (
          <div>
            <div style={{ background:COLORS.surface, border:`1px solid ${COLORS.border}`, borderRadius:14, padding:'18px 18px', marginBottom:16 }}>
              ...
              <div style={{ fontSize:14, lineHeight:1.85, color:COLORS.text, fontStyle:'italic', borderLeft:`3px solid ${color}`, paddingLeft:14 }}>{step.lesson}</div>
            </div>

            {(isAI || isHybrid) && (
              <div style={{ background:COLORS.surface, border:`1px solid ${COLORS.violet}33`, borderRadius:14, padding:'16px 18px', marginBottom:16 }}>
                <div style={{ fontSize:11, color:COLORS.violet, fontWeight:700, marginBottom:10 }}>🤖 Ask {guru.name} a question about this lesson</div>
                <textarea value={aiInput} onChange={e => setAiInput(e.target.value)} placeholder={`Ask ${guru.name} anything...`} style={S.textarea} rows={3} />
                <button onClick={handleAskGuru} disabled={aiLoading || !aiInput.trim()} style={{ ...S.aiBtn, background: aiLoading || !aiInput.trim() ? COLORS.surface2 : COLORS.violet+'22', color:COLORS.violet, border:`1px solid ${COLORS.violet}44` }}>
            ...
            <PrimaryBtn color={color} onClick={() => setPhase('action')} style={{ width:'100%' }}>Ready for the Practical Action →</PrimaryBtn>
          </div>
        )}
        ...
        {phase === 'action' && (
          <div>
            <div style={{ background:COLORS.surface, border:`1px solid ${color}33`, borderRadius:14, padding:'18px 18px', marginBottom:16 }}>
              <div style={{ fontSize:10, color, letterSpacing:1.5, textTransform:'uppercase', marginBottom:8, fontWeight:700 }}>⚡ Your Practical Action</div>
            ...
            <div style={{ background:COLORS.surface2, border:`1px solid ${COLORS.border}`, borderRadius:12, padding:'12px 16px', marginBottom:16, display:'flex', alignItems:'center', gap:10 }}>
              ...
              <div>
                ...
                <div style={{ fontSize:11, color:'#555' }}>Set a timer and fully commit before continuing.</div>
              </div>
            </div>

            {!isCompleted && !completed ? (
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                <PrimaryBtn color={color} onClick={markComplete} style={{ width:'100%' }}>✓ I completed this action</PrimaryBtn>
                <GhostBtn onClick={() => setPhase('lesson')} style={{ width:'100%' }}>← Back to lesson</GhostBtn>
              </div>
            ) : (
              <div>
                <div style={{ background:color+'15', border:`1px solid ${color}44`, borderRadius:12, padding:'16px 18px', marginBottom:14, textAlign:'center' }}>
                  ...
                  <div style={{ fontSize:12, color:'#666' }}>You completed Step {step.stepNumber} of {guru.roadmap.length}.</div>
                </div>
...
const S = {
  textarea:{ width:'100%', padding:'11px 14px', borderRadius:10, border:`1px solid ${COLORS.border2}`, background:COLORS.surface2, color:COLORS.text, fontSize:13, fontFamily:'inherit', resize:'vertical', boxSizing:'border-box', lineHeight:1.6, marginBottom:8 },
  aiBtn:{ width:'100%', padding:'10px', borderRadius:9, fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit', border:'none' },
};

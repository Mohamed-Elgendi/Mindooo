...
import { TopBar, BackButton, Tag, COLORS, MiniMindMap } from '../shared/UI.jsx';
...
export function NobelDomainScreen({ m }) {
  ...
  const [tab, setTab] = useState('thinkers');
  ...
  return (
    <div className="section-scroll">
      <div className="section-content">
        <TopBar left={<BackButton onClick={() => m.navigate('nobel-mind')} />} center={<span style={{ color, fontWeight:800, fontSize:15 }}>{domain.icon} {domain.label}</span>} />
        ...
        <div style={{ display:'flex', gap:8, marginBottom:18 }}>
          {due > 0 && <button onClick={() => m.startSession('nobel-mind', 'review')} style={{ flex:1, padding:'12px', borderRadius:10, border:'none', background:COLORS.teal, color:'#07080f', fontWeight:800, fontSize:13, cursor:'pointer', fontFamily:'inherit' }}>📦 Review ({due})</button>}
          {newCards > 0 && <button onClick={() => m.startSession('nobel-mind', 'new')} style={{ flex:1, padding:'12px', borderRadius:10, border:'none', background:color, color:'#07080f', fontWeight:800, fontSize:13, cursor:'pointer', fontFamily:'inherit' }}>🔥 Learn ({newCards})</button>}
          {due === 0 && newCards === 0 && <div style={{ flex:1, padding:'12px', borderRadius:10, background:COLORS.surface2, color:'#555', fontSize:13, textAlign:'center' }}>✓ All caught up</div>}
        </div>

        <div style={{ display:'flex', gap:6, marginBottom:18 }}>
          {[['thinkers','🧑‍🎓 Thinkers'],['cards','🃏 Cards'],['exercises','💪 Exercises']].map(([id,lbl]) => (
            <button key={id} onClick={() => setTab(id)} style={{ flex:1, padding:'9px', borderRadius:10, border:`1px solid ${tab===id?color:COLORS.border2}`, background:tab===id?color+'22':COLORS.surface2, color:tab===id?color:'#555', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>{lbl}</button>
          ))}
        </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {domain.thinkers.map((t,i) => (
              <div key={i} style={{ background:COLORS.surface, border:`1px solid ${COLORS.border}`, borderRadius:14, padding:'16px 18px' }}>
                ...
                <div style={{ fontSize:11, color:'#555', marginBottom:8 }}>{t.years} · {t.nationality} · {t.field}</div>
                ...
                <div key={card.id} style={{ background:COLORS.surface, border:`1.5px solid ${cardColor}33`, borderRadius:16, padding:'18px 18px' }}>
                  <div style={{ display:'flex', gap:10, alignItems:'center', marginBottom:12 }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ fontSize:10, color:'#444', marginTop:2 }}>{card.thinkers?.join(' · ')}</div>
                    </div>
                  ...
                  <div style={{ fontSize:13, lineHeight:1.85, color:'#ccc', fontStyle:'italic', marginBottom:14, borderLeft:`3px solid ${cardColor}44`, paddingLeft:14 }}>{card.unit}</div>
                  <div style={{ display:'flex', gap:5, flexWrap:'wrap', marginBottom: card.mindmap?12:0 }}>
                    {card.keyPoints.map((kp,j) => <div key={j} style={{ background:cardColor+'12', border:`1px solid ${cardColor}33`, borderRadius:6, padding:'3px 10px', fontSize:11, color:cardColor+'cc', fontWeight:600 }}>{kp}</div>)}
                  </div>
                  {card.mindmap && m.settings.showMindmap && <div style={{ background:'#060810', borderRadius:10, padding:'10px', border:`1px solid ${COLORS.border}`, marginTop:10 }}><MiniMindMap data={card.mindmap} /></div>}
                </div>
              );
            })}
          </div>

  ...
  return (
    <div className="section-scroll">
      <div className="section-content">
        <TopBar left={<BackButton onClick={() => m.navigate('guru-center')} />} center={<span style={{ color, fontWeight:800, fontSize:15 }}>{guru.icon} {guru.name}</span>} />
          ...
          <div style={{ marginBottom:14 }}>
            ...
            <div style={{ display:'flex', gap:6 }}>
              {[['pre-written','📄 Pre-written'],['ai','🤖 AI'],['hybrid','⚡ Hybrid']].map(([val,lbl]) => (
                <button key={val} onClick={() => m.setGuruGuidanceMode(val)} style={{ flex:1, padding:'8px 6px', borderRadius:8, border:`1px solid ${m.guruGuidanceMode===val?color:COLORS.border2}`, background:m.guruGuidanceMode===val?color+'22':COLORS.surface2, color:m.guruGuidanceMode===val?color:'#666', fontSize:11, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>{lbl}</button>
              ))}
            </div>
          </div>
          {nextStep && <PrimaryBtn color={color} onClick={() => openStep(nextStep)} style={{ width:'100%' }}>{completed.length === 0 ? `Start Step 1 — ${nextStep.title}` : `Continue → Step ${nextStep.stepNumber}`}</PrimaryBtn>}
          {!nextStep && total > 0 && <div style={{ textAlign:'center', color, fontWeight:700, fontSize:14 }}>🏆 All steps complete!</div>}
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {guru.roadmap.map(step => {
            const done = completed.includes(step.id);
            const isCurrent = nextStep?.id === step.id;
            return (
              <div key={step.id} onClick={() => openStep(step)} style={{ background:COLORS.surface, border:`1.5px solid ${done?color+'55':isCurrent?color+'33':COLORS.border}`, borderRadius:12, padding:'14px 16px', cursor:'pointer', opacity:done?0.65:1, display:'flex', gap:12, alignItems:'flex-start' }}>
                <div style={{ width:28, height:28, borderRadius:'50%', flexShrink:0, background:done?color:isCurrent?color+'22':COLORS.surface2, border:`1.5px solid ${done||isCurrent?color:COLORS.border2}`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:800, color:done?COLORS.bg:isCurrent?color:'#555' }}>

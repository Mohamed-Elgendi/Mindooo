...
import { TopBar, BackButton, SectionHeader, Tag, COLORS, PrimaryBtn, GhostBtn, EmptyState, MiniMindMap } from '../shared/UI.jsx';
...
export function CurriculumScreen({ m }) {
    ...
    return (
      <div className="section-scroll">
        <div className="section-content">
          ...
          <EmptyState icon="📭" title="Curriculum not found" sub="This curriculum has not been registered yet." />
        </div>
      </div>
    );
  }
  ...
  function attemptStart(sessionId) {
    ...
    if (canStart) {
      m.startSession(curriculumId, sessionId);
    } else {
      setPendSession(sessionId);
      setSkipVal('');
      setSkipModal(true);
    }
  }
  ...
  function confirmSkip() {
    if (skipVal !== 'SKIP') return;
    setSkipModal(false);
    m.startSession(curriculumId, pendSession);
  }
  ...
  return (
    <div className="section-scroll">
      <div className="section-content">
        ...
        <TopBar
          left={<BackButton onClick={() => m.navigate('knowledge-archive')} />}
          center={<span style={{ color, fontWeight: 800, fontSize: 15 }}>{icon} {label}</span>}
          right={<button onClick={() => setShowCards(s => !s)} style={{ background: 'none', border: 'none', color: '#555', fontSize: 12, cursor: 'pointer' }}>{showCards ? 'Hide' : '📋 Cards'}</button>}
        />

        {/* Stats */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, marginBottom: 20 }}>
          {[
            { l: 'Total',    v: cards.length, c: '#666' },
            { l: 'New',      v: newCards,     c: COLORS.sage },
            { l: 'In Boxes', v: inBoxes,      c: COLORS.teal },
            { l: 'Due',      v: due,          c: due > 0 ? COLORS.rose : COLORS.teal },
          ].map(s => (
        ...
        {newCards === 0 && due === 0 && cards.length > 0 && (
          <div style={{ background: '#0a1f0a', border: `1px solid ${COLORS.sage}44`, borderRadius: 12, padding: '12px 16px', fontSize: 13, color: COLORS.sage, marginBottom: 16 }}>
            ✓ All cards in boxes and not yet due. Come back later!
          </div>
        )}
          ...
          {SESSIONS.map((s, i) => {
            ...
            return (
              <div key={s.id} style={{
                ...
                border:       `1.5px solid ${done ? s.color + '55' : canStart ? s.color + '22' : COLORS.border}`,
                borderRadius: 16,
                padding:      '16px 16px',
                opacity:      done ? 0.6 : 1,
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', flex: 1 }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 800, color: done ? s.color : COLORS.text, fontSize: 14 }}>
                        {i + 1}. {s.label} {done ? '✓' : ''}
                      </div>
                      <div style={{ fontSize: 11, color: '#555', marginTop: 3 }}>{s.desc}</div>
                      {!canStart && !done && prev && (
                        <div style={{ fontSize: 11, color: COLORS.rose, marginTop: 5 }}>
                          ⚠ Complete {blockingLabel} first — or type SKIP
                        </div>
                      )}
                      {canStart && !done && !hasCards && (
                        <div style={{ fontSize: 11, color: '#555', marginTop: 5 }}>
                          {m.boxes.getEmptySessionReason(cards, s.id)}
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div style={{ flexShrink: 0, marginLeft: 10 }}>
                    {!done ? (
                      <button
                        onClick={() => attemptStart(s.id)}
                        style={{
                          padding:      '9px 16px',
                          borderRadius: 10,
                          border:       canStart ? 'none' : `1px solid ${COLORS.rose}44`,
                          background:   canStart && hasCards ? s.color : canStart ? COLORS.surface2 : COLORS.surface2,
                          color:        canStart && hasCards ? COLORS.bg : canStart ? '#555' : COLORS.rose,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 6, marginBottom: 20 }}>
          {boxes.map(b => (
            <div key={b.box} style={{
              ...
              {b.dueCount > 0 && (
                <div style={{ position: 'absolute', top: 4, right: 4, width: 5, height: 5, borderRadius: '50%', background: COLORS.rose }} />
              )}
              <div style={{ fontSize: 9, color: '#444' }}>B{b.box}</div>
              <div style={{ fontSize: 14, fontWeight: 900, color: b.dueCount > 0 ? COLORS.rose : COLORS.teal, marginTop: 2 }}>
                {b.count || '—'}
              </div>
        ...
        {showCards && (
          <>
            <SectionHeader label={`Cards (${cards.length})`} action="+ Add" onAction={() => {}} />
            {cards.length === 0 ? (
              <EmptyState icon="📭" title="No cards yet" sub="Add cards to this curriculum or use the AI generator." />
            ) : (
                  ...
                  return (
                    <div key={card.id} style={{ background: COLORS.surface, border: `1px solid ${cardColor}33`, borderRadius: 14, padding: '14px 16px' }}>
                      ...
                      {m.settings.showKeypoints && card.keyPoints && (
                        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 8 }}>
                          {card.keyPoints.map((kp, i) => (
                            <div key={i} style={{ background: cardColor + '15', border: `1px solid ${cardColor}33`, borderRadius: 6, padding: '3px 9px', fontSize: 10, color: cardColor, fontWeight: 600 }}>{kp}</div>
                          ))}
                        </div>
                      )}
                      {m.settings.showMindmap && card.mindmap && (
                        <div style={{ background: '#060810', borderRadius: 10, padding: '10px 12px', border: `1px solid ${COLORS.border}`, marginBottom: 8 }}>
                      ...
                      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                        ...
                        <Tag color={isDue && card.box > 0 ? COLORS.rose : COLORS.border2}>
                          {card.box === 0 ? 'New' : isDue ? '⏰ Due' : `⏳ ${card.dueAt ? Math.ceil((new Date(card.dueAt) - new Date()) / 86400000) + 'd' : '—'}`}
                        </Tag>
                      </div>
                    </div>
                  );
        ...
        <div style={{ marginTop: 24, textAlign: 'center' }}>
          <button onClick={m.resetDailyProgress} style={{ background: 'none', border: `1px solid ${COLORS.border}`, borderRadius: 8, color: '#444', fontSize: 11, padding: '6px 14px', cursor: 'pointer' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.92)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border2}`, borderRadius: 20, padding: '32px 24px', maxWidth: 400, width: '100%', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 13, color: '#666', lineHeight: 1.7, marginBottom: 16 }}>
              Your memory system works best in strict daily order. Skipping weakens long-term retention.<br />
              Type <strong style={{ color: COLORS.rose, letterSpacing: 3 }}>SKIP</strong> to confirm.
            </div>
            <input
              style={{ width: '100%', padding: '13px', borderRadius: 10, border: `1px solid ${COLORS.border2}`, background: COLORS.surface2, color: COLORS.text, fontSize: 16, fontFamily: 'inherit', textAlign: 'center', letterSpacing: 3, boxSizing: 'border-box', marginBottom: 14 }}
              value={skipVal}
              onChange={e => setSkipVal(e.target.value)}
              placeholder="Type SKIP here…"
              autoFocus
            />
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                onClick={confirmSkip}
                disabled={skipVal !== 'SKIP'}
                style={{ flex: 1, padding: '12px', borderRadius: 10, border: `1px solid ${skipVal === 'SKIP' ? COLORS.rose : COLORS.border2}`, background: skipVal === 'SKIP' ? COLORS.rose + '22' : COLORS.surface2, color: skipVal === 'SKIP' ? COLORS.rose : '#444', fontWeight: 700, fontSize: 13, cursor: skipVal === 'SKIP' ? 'pointer' : 'not-allowed', fontFamily: 'inherit' }}
              >Confirm Skip</button>
              <button
                onClick={() => setSkipModal(false)}
                style={{ flex: 1, padding: '12px', borderRadius: 10, border: `1px solid ${COLORS.border2}`, background: COLORS.surface2, color: '#666', fontWeight: 600, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}
              >Cancel</button>
            </div>
          </div>
        </div>

...
import { Tag, StatBox, SectionHeader } from '../shared/UI.jsx';
...
export function HomeScreen({ m }) {
  ...
  function handleDrop(e, targetId) {
    ...
    m.reorderSections(all.map((s,i) => ({ ...s, order: i })));
          ...
          {sections.map((sec, idx) => {
            ...
            return (
              <div key={sec.id} draggable
                ...
                style={{ ...S.sectionCard, borderColor: isOver ? cfg.color : `${cfg.color}33`, opacity: isDragging ? 0.4 : 1, background: isOver ? `${cfg.color}10` : '#0e1018', cursor:'pointer' }}
              >
                <div style={S.dragHandle} onClick={e => e.stopPropagation()}>⠿</div>
                <div style={S.cardTop}>
                  <div style={{ ...S.cardIcon, background:`${cfg.color}18`, border:`1px solid ${cfg.color}44` }}>{sec.icon}</div>
                  <div style={{ flex:1 }}>
        ...
        {m.sections.some(s => !s.visible) && (
          <div style={S.hiddenWrap}>
            ...
            {m.sections.filter(s => !s.visible).map(s => (
              <button key={s.id} style={S.showBtn} onClick={() => m.toggleSection(s.id)}>{s.icon} {s.label} (show)</button>
            ))}
          </div>
        )}
...
function SectionQuickStats({ m, sectionId, color }) {
  if (sectionId === 'knowledge-archive') {
    ...
    return <div style={S.quickStats}>{due > 0 && <Tag color="#f0657a">{due} due</Tag>}{nw > 0 && <Tag color="#3dd9c4">{nw} new</Tag>}{due === 0 && nw === 0 && <Tag color="#333">All caught up ✓</Tag>}</div>;
  }
  if (sectionId === 'nobel-mind') {
    ...
    return <div style={S.quickStats}><Tag color={color}>{cards.length} cards</Tag>{due > 0 && <Tag color="#f0657a">{due} due</Tag>}</div>;
  }
  if (sectionId === 'guru-center') {
    const started = Object.keys(m.completedGuruSteps).length;
    return <div style={S.quickStats}><Tag color={color}>{m.gurus.length} gurus</Tag>{started > 0 && <Tag color="#3dd9c4">{started} in progress</Tag>}</div>;
  }
  if (sectionId === 'ai-mind-lab') return <div style={S.quickStats}><Tag color={color}>AI Coach</Tag><Tag color={color}>Card Generator</Tag></div>;
  return null;
}

function DailyProgressRow({ m }) {
  const SESSIONS = [{ id:'review', icon:'📦', color:'#3dd9c4' }, { id:'new', icon:'🔥', color:'#f0657a' }, { id:'preview', icon:'🔭', color:'#f5c842' }];
            ...
            <div style={{ display:'flex', gap:10 }}>
              {SESSIONS.map(s => (
                <div key={s.id} style={{ display:'flex', alignItems:'center', gap:3 }}>
                  <div style={{ width:8, height:8, borderRadius:'50%', background: p[`${s.id}Done`] ? s.color : '#1e2130', border:`1.5px solid ${p[`${s.id}Done`] ? s.color : '#2a2d3e'}` }} />
...
const S = {
  ...
  dragHandle:{ position:'absolute', top:14, right:14, color:'#2a2d3e', fontSize:14, cursor:'grab' },

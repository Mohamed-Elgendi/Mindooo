  ...
  return (
    <div className="section-scroll">
      <div className="section-content">
        <TopBar left={<BackButton onClick={() => m.navigate('home')} />} center={<div style={{ fontWeight:800, fontSize:16, color:COLORS.rose }}>🏆 Nobel Mind</div>} />

        <div style={{ fontSize:13, color:'#666', marginBottom:20, lineHeight:1.6 }}>
          Six domains of world-class thinking — mental models and frameworks that shape how you reason.
        </div>

        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, marginBottom:20 }}>
          {[{ l:'Total Cards', v:cards.length, c:COLORS.rose }, { l:'Due Now', v:due, c: due>0?COLORS.rose:COLORS.teal }, { l:'Mastered', v:mastered, c:COLORS.gold }].map(s => (
        ...
        {(due > 0 || nw > 0) && (
          <div style={{ display:'flex', gap:8, marginBottom:20 }}>
            {due > 0 && <button onClick={() => m.startSession('nobel-mind', 'review')} style={{ flex:1, padding:'13px', borderRadius:12, border:'none', background:COLORS.teal, color:'#07080f', fontWeight:800, fontSize:13, cursor:'pointer', fontFamily:'inherit' }}>📦 Review ({due})</button>}
            {nw > 0 && <button onClick={() => m.startSession('nobel-mind', 'new')} style={{ flex:1, padding:'13px', borderRadius:12, border:'none', background:COLORS.rose, color:'#07080f', fontWeight:800, fontSize:13, cursor:'pointer', fontFamily:'inherit' }}>🔥 Learn ({nw})</button>}
          </div>
        )}
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          {m.nobelDomains.map(domain => {
            ...
            const domainCards = cards.filter(c => domain.cards.some(dc => dc.id === c.id));
            ...
            return (
              <Card key={domain.id} color={color} onClick={() => m.setActiveDomain(domain.id)} style={{ cursor:'pointer' }}>
                <div style={{ display:'flex', gap:14, alignItems:'flex-start', marginBottom:12 }}>
                  <div style={{ fontSize:28, width:50, height:50, borderRadius:12, flexShrink:0, background:color+'18', border:`1px solid ${color}44`, display:'flex', alignItems:'center', justifyContent:'center' }}>{domain.icon}</div>
                  <div style={{ flex:1 }}>
                ...
                <div style={{ display:'flex', gap:5, flexWrap:'wrap', marginBottom:10 }}>
                  {domain.thinkers.slice(0,3).map((t,i) => <Tag key={i} color={color}>{t.name.split(' ').pop()}</Tag>)}

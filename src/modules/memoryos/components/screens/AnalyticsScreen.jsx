...
import { TopBar, BackButton, SectionHeader, Tag, COLORS } from '../shared/UI.jsx';
  ...
  if (!a) {
    return (
      <div className="section-scroll"><div className="section-content">
        <TopBar left={<BackButton onClick={() => m.navigate('home')} />} center={<div style={{ fontWeight:800, fontSize:16, color:COLORS.teal }}>📊 Analytics</div>} />
        <div style={{ textAlign:'center', padding:'48px 0', color:'#555', fontSize:13 }}>Loading analytics...</div>
      </div></div>
    );
  }
  ...
  return (
    <div className="section-scroll">
      <div className="section-content">
        <TopBar left={<BackButton onClick={() => m.navigate('home')} />} center={<div style={{ fontWeight:800, fontSize:16, color:COLORS.teal }}>📊 Analytics</div>} right={<button onClick={m.progress.refreshAnalytics} style={{ background:'none', border:'none', color:'#555', fontSize:13, cursor:'pointer' }}>↻</button>} />
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:8, marginBottom:18 }}>
          {[{ l:'Total Reviews', v:a.totalReviews, c:COLORS.teal }, { l:'Mastery Rate', v:`${a.masteryRate}%`, c:COLORS.sage }, { l:'Cards Mastered', v:a.cardsMastered, c:COLORS.gold }, { l:'Cards Due', v:a.cardsDue, c: a.cardsDue>0?COLORS.rose:COLORS.teal }].map(s => (
        ...
        <div style={{ background:COLORS.surface, border:`1px solid ${COLORS.border}`, borderRadius:14, padding:'16px 18px', marginBottom:18 }}>
          ...
          {[{ l:'Owned', v:a.totalOwned, c:COLORS.sage }, { l:'Almost', v:a.totalAlmost, c:COLORS.gold }, { l:'Failed', v:a.totalFailed, c:COLORS.rose }].map(s => {
            const pct = a.totalReviews > 0 ? Math.round((s.v / a.totalReviews) * 100) : 0;
            return (
              <div key={s.l} style={{ marginBottom:12 }}>
                <div style={{ display:'flex', justifyContent:'space-between', marginBottom:5 }}><span style={{ fontSize:12, color:'#888' }}>{s.l}</span><span style={{ fontSize:12, color:s.c, fontWeight:700 }}>{s.v} ({pct}%)</span></div>
                <div style={{ height:5, background:COLORS.surface2, borderRadius:3, overflow:'hidden' }}><div style={{ height:'100%', width:`${pct}%`, background:s.c, borderRadius:3 }} /></div>
              </div>
            );
          })}
        </div>
              ...
              return (
                <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:4 }}>
                  <div style={{ width:'100%', height:64, display:'flex', flexDirection:'column', justifyContent:'flex-end' }}>
                    <div style={{ width:'100%', borderRadius:'4px 4px 0 0', height:`${Math.max(pct, day.total>0?8:2)}%`, background: day.total>0?`linear-gradient(to top, ${COLORS.teal}, ${COLORS.sage})`:COLORS.border, minHeight: day.total>0?6:2 }} />
                  </div>
                ...
                return (
                  <div key={c.curriculumId} style={{ background:COLORS.surface, border:`1px solid ${COLORS.border}`, borderRadius:12, padding:'14px 16px' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
                      <div style={{ fontWeight:700, fontSize:13, color }}>{c.icon} {c.label}</div>
                      <Tag color={color}>{c.masteryRate}% mastery</Tag>
                    </div>
                    <div style={{ height:4, background:COLORS.surface2, borderRadius:2, overflow:'hidden', marginBottom:10 }}><div style={{ height:'100%', width:`${c.masteryRate}%`, background:color, borderRadius:2 }} /></div>
                    <div style={{ display:'flex', gap:8, flexWrap:'wrap', fontSize:11, color:'#555' }}>
                      <span>{c.totalCards} cards</span><span>·</span><span>{c.inBoxes} in boxes</span><span>·</span><span style={{ color:COLORS.gold }}>{c.mastered} mastered</span>
                    </div>
                  </div>
                );
              })}

...
const SESSIONS = [
  { id: 'review',  label: 'Box Review',  icon: '📦', color: COLORS.teal },
  { id: 'new',     label: 'New',         icon: '🔥', color: COLORS.rose },
  { id: 'preview', label: 'Preview',     icon: '🔭', color: COLORS.gold },
];
...
export function ArchiveScreen({ m }) {
  ...
  // Global stats across all curricula
    ...
    <div className="section-scroll">
      <div className="section-content">
        ...
        <TopBar
          ...
          center={<div style={{ fontWeight: 800, fontSize: 16, color: COLORS.teal }}>📚 Knowledge Archive</div>}
        />
        ...
        <div style={{ fontSize: 13, color: '#666', marginBottom: 16, lineHeight: 1.6 }}>
          ...
          Build a bibliotheque of knowledge stored permanently in your memory.
        </div>

        {/* Global quick stats */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 20 }}>
          {[
            { label: 'Due Now',  value: totalDue, color: totalDue > 0 ? COLORS.rose : COLORS.teal },
            { label: 'New',      value: totalNew, color: COLORS.sage },
            { label: 'In Boxes', value: totalIn,  color: COLORS.teal },
          ].map(s => (
        ...
        {/* Curricula list */}
        <SectionHeader label="Pre-built Curricula" action="+ Add Custom" onAction={() => m.navigate('curriculum-detail', { curriculumId: 'custom' })} />
        ...
        {curricula.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
            {curricula.map(c => {
              ...
              return (
                <Card key={c.id} color={color}
                  onClick={() => {
                    m.setActiveCurriculum(c.id);
                    m.navigate('curriculum-detail', { curriculumId: c.id });
                  }}
                  style={{ cursor: 'pointer' }}
                >
                  <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: 12 }}>
                    <div style={{
                      fontSize: 26, width: 46, height: 46, borderRadius: 10, flexShrink: 0,
                    ...
                    {SESSIONS.map(s => (
                      <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        <div style={{
                          ...
                          background: p[`${s.id}Done`] ? s.color : COLORS.border2,
                          border: `1.5px solid ${p[`${s.id}Done`] ? s.color : COLORS.border2}`,
                          transition: 'all .2s',
                        }} />
        ...
        <Card color={COLORS.rose} onClick={() => m.navigate('nobel-mind')} style={{ cursor: 'pointer', marginBottom: 20 }}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: '#555', marginTop: 2 }}>6 domains · {nobelCards.length} cards · World-class thinking frameworks</div>
            </div>
          </div>
        </Card>

        {/* Add custom card */}
        <SectionHeader label="Custom Cards" action="+ Add Card" onAction={() => m.navigate('curriculum-detail', { curriculumId: 'custom', addCard: 'true' })} />

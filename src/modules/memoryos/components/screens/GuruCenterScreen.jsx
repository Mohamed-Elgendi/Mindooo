  ...
  return (
    <div className="section-scroll">
      <div className="section-content">
        <TopBar left={<BackButton onClick={() => m.navigate('home')} />} center={<div style={{ fontWeight:800, fontSize:16, color:COLORS.gold }}>🧠 Guru Center</div>} />
        <div style={{ fontSize:13, color:'#666', marginBottom:20, lineHeight:1.6 }}>
          Select a guru and follow their step-by-step training path. Each step delivers one lesson and one practical action.
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          {m.gurus.map(guru => {
            ...
            return (
              <Card key={guru.id} color={color} onClick={() => m.setActiveGuru(guru.id)} style={{ cursor:'pointer' }}>
                <div style={{ display:'flex', gap:14, alignItems:'flex-start' }}>
                  <div style={{ fontSize:32, width:52, height:52, borderRadius:12, background:color+'18', border:`1px solid ${color}44`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>{guru.icon}</div>
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ fontSize:12, color:'#555', lineHeight:1.5, marginBottom:10, fontStyle:'italic' }}>"{guru.tagline}"</div>

...
export function SessionDoneScreen({ m }) {
  ...
  const grade = pct >= 90 ? { icon:'🏆', label:'Outstanding', color:COLORS.gold } : pct >= 70 ? { icon:'✅', label:'Strong', color:COLORS.sage } : pct >= 50 ? { icon:'⚡', label:'Progressing', color:COLORS.amber } : { icon:'🔁', label:'Keep Going', color:COLORS.rose };
    ...
    <div className="section-scroll">
      <div className="section-content">
        ...
        <div style={{ display:'flex', justifyContent:'center', marginBottom:28 }}>
          <div style={{ width:110, height:110, borderRadius:'50%', background:`conic-gradient(${grade.color} ${pct*3.6}deg, #141620 0deg)`, display:'flex', alignItems:'center', justifyContent:'center' }}>
            <div style={{ width:86, height:86, borderRadius:'50%', background:'#07080f', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginBottom:24 }}>
          {[{ l:'Total', v:stats.total, c:'#666' }, { l:'Owned', v:stats.owned, c:COLORS.sage }, { l:'Almost', v:stats.almost, c:COLORS.gold }, { l:'Failed', v:stats.failed, c:COLORS.rose }, { l:'Grade', v:grade.label, c:grade.color }, { l:'Phase', v: cfg?.sessionType==='new'?'6-Ph':'4-Ph', c:color }].map(s => (
        ...
        <div style={{ background:color+'10', border:`1px solid ${color}33`, borderRadius:14, padding:'16px 18px', marginBottom:24, textAlign:'center' }}>
          <div style={{ fontSize:13, color:'#888', lineHeight:1.7 }}>
            {pct >= 80 ? 'Excellent session. Your memory traces are deepening.' : pct >= 60 ? 'Good progress. Cards marked Almost will return at the same interval.' : 'Every session builds the foundation. Failed cards reset to Box 1.'}
          </div>
        </div>
        ...
        {nextSessions.length > 0 && (
          <div style={{ marginBottom:20 }}>
            ...
            {nextSessions.map(ns => (
              <button key={ns.id} onClick={() => m.startSession(curriculumId, ns.id)} style={{ width:'100%', padding:'14px 20px', borderRadius:12, border:`1px solid ${COLORS.border2}`, background:COLORS.surface, color:COLORS.text, fontWeight:700, fontSize:14, cursor:'pointer', fontFamily:'inherit', marginBottom:8, display:'flex', alignItems:'center', justifyContent:'center', gap:10 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          <PrimaryBtn color={color} onClick={() => { s.endSession(); m.navigate('curriculum-detail', { curriculumId }); }} style={{ width:'100%' }}>Back to Curriculum</PrimaryBtn>
          <GhostBtn onClick={() => { s.endSession(); m.navigate('home'); }} style={{ width:'100%' }}>← Home</GhostBtn>
        </div>
      </div>
    </div>
  );
